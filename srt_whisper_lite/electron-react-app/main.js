...
ipcMain.handle('get-language-config', async () => {
  try {
    const configPath = path.join(__dirname, 'language_config.json')
    if (fs.existsSync(configPath)) {
      const config = JSON.parse(fs.readFileSync(configPath, 'utf-8'))
      console.log('載入語言配置:', config)
      return config
    }
    
    // 備用位置
    const publicConfigPath = path.join(__dirname, 'react-app', 'public', 'language_config.json')
    if (fs.existsSync(publicConfigPath)) {
      const config = JSON.parse(fs.readFileSync(publicConfigPath, 'utf-8'))
      console.log('從public目錄載入語言配置:', config)
      return config
    }
    
    // 預設配置
    console.log('使用預設語言配置')
    return {
      selectedLanguage: 'en',
