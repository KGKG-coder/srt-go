            ...
            try:
                ...
                if hasattr(self, 'audio_file') and self.audio_file:
                    ...
                    for seg in fixed_segments:
                        ...
                        # 如果有 Whisper 原始數據，添加信心分數
                        if hasattr(seg, 'no_speech_prob'):
                            segment_dict['no_speech_prob'] = getattr(seg, 'no_speech_prob', 0.5)
                        if hasattr(seg, 'avg_logprob'):
                            segment_dict['avg_logprob'] = getattr(seg, 'avg_logprob', -1.0)
                    ...
                    # 轉換回 TextSegment 格式
                    fixed_segments = []
                    for seg_dict in filtered_dicts:
                        seg = TextSegment(
                    ...
                    logger.info(f"SubEasy 多層過濾完成，處理了 {len(fixed_segments)} 個段落")
                else:
                    logger.warning("音頻文件路徑未提供，跳過 SubEasy 多層過濾")
            except ImportError:
                logger.warning("SubEasy 多層過濾模組未找到，使用標準處理")
            except Exception as e:
                logger.error(f"SubEasy 多層過濾失敗: {e}")
...
def optimize_subtitle_segments(segments: List[Dict[str, Any]], 
                             language: str = "auto",
                             model_size: str = "medium") -> List[Dict[str, Any]]:
    """
    優化字幕片段的便捷函數
